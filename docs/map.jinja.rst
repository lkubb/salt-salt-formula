...
We will see a quick setup to configure the ``salt`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``salt`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``salt`` formulas under ``/srv/salt/salt/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/salt/parameters/dns:domain/
    ln -s dns:domain /srv/salt/salt/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/salt/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/salt/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``salt`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``salt`` formulas under ``/srv/salt/salt/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/salt/parameters/roles
...
We create a configuration for the role ``salt/server`` in ``/srv/salt/salt/parameters/roles/salt/server.yaml``:
...
We create another configuration for the role ``salt/client`` in ``/srv/salt/salt/parameters/roles/salt/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles salt/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles salt/client
...
.. code-block:: console
    ...
    mv /srv/salt/salt/parameters/dns:domain/  '/srv/salt/salt/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    {%- from tplroot ~ "/map.jinja" import mapdata as salt with context %}

    test-does-nothing-but-display-salt-as-json:
      test.nop:
        - name: {{ salt | json }}
...
Here is an example based on `template-formula/salt/config/file.sls`_:
    ...
    salt-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='salt-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ salt.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``salt`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ salt['winner'] }}
...
.. _template-formula/salt/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/salt/config/file.sls
